...
| --- | --- | --- | --- | --- |
| p | `SVGProps<SVGSVGElement>` | — | yes | Caller-supplied `<svg>` props (e.g. `className`, `style`); spread onto the inner `Svg` so consumers can recolor or resize without forking the icon. |
...
<!-- fill:file:diagrams -->
:::note
Pure presentational components — each icon is a single SVG return value with no branching. No control flow worth diagramming.
:::
<!-- /fill:file:diagrams -->
