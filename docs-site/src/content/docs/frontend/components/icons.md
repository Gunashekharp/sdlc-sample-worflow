...
| --- | --- | --- | --- | --- |
| p | `SVGProps<SVGSVGElement>` | — | yes | Standard SVG props (e.g. `className`, `width`, `style`) spread onto the underlying `<svg>`, letting callers size and color the icon via `currentColor`. |
...
<!-- fill:file:diagrams -->
:::note
A flat set of stateless presentational SVG components sharing one `Svg` wrapper — no control flow, async work, or state transitions to diagram.
:::
<!-- /fill:file:diagrams -->
