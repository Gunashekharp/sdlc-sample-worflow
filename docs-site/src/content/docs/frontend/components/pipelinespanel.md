...
| Suite | Test | Asserts |
| --- | --- | --- |
| <PipelinesPanel /> | renders pipelines returned by the API | With `fetch` stubbed to resolve sample data, both pipeline names (`CI · build & test`, `E2E suite`) appear in the rendered list. |
| <PipelinesPanel /> | shows an error state when the API is unreachable | With `fetch` rejected with `network down`, the "could not reach the api" message renders inside the panel. |
