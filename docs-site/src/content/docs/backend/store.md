...
| --- | --- | --- | --- | --- |
| agents | `Agent[]` | — | yes | The agent catalogue served by `listAgents`/`getAgent`; held in a closure (typically `SEED_AGENTS`). |
| kpis | `Kpi[]` | — | yes | The KPI list returned by `listKpis`; held in the same closure (typically `SEED_KPIS`). |
