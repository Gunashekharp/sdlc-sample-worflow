...
| --- | --- | --- | --- | --- |
| app | `Express` | — | yes | The Express app to attach the `/api/*` route handlers onto; mutated in place. |
| deps | `AppDeps` | — | yes | The injected `store` and `cicd` provider the handlers call to fetch agent, KPI, and pipeline data. |
