...
export function buildRootDepGraph(root: SourceRoot): string | null {
  ...
  // Mermaid v11 wants the diagram type on the first non-empty line.
  // Putting `%%` comments before `flowchart` makes some renderers throw a
  // syntax error. Comments live inside the body instead.
  const lines: string[] = []
